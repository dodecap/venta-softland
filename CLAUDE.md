...
Desde la 0.44.0 el sistema se instala en cualquier empresa Softland sin que
nadie edite un archivo. Lo que hay que saber antes de tocar nada:

- **Un repositorio, N instalaciones. No se copia el repositorio por cliente.**
  Lo de cada empresa ya vive fuera del código: la conexión en
  `storage/app/private/softland.json`, la identidad y las reglas en
  `ventas.config`, y los folios, atributos y giros en la propia base Softland.
  Una copia por cliente es un merge por cada arreglo y, a los seis meses, cada
  cliente corriendo un código distinto.
- **Ningún nombre de empresa en el código.** `INNOVAGES` sólo puede aparecer en
  comentarios que citan una medición. Ni en un valor por omisión de un
  formulario, ni en `config/`, ni en lo que imprime un comando.
...
- **La comprobación del servidor va antes del formulario**, no después de
  ...
  driver» no le dice qué hacer.
- **El servidor no escribe direcciones absolutas, tampoco al redirigir.** La
  regla de `/app` vale para todo: detrás de un proxy, `redirect('/setup')`
  generaba `…/venta-softland`, que da **404**, y era la primera página que abría
  quien instalaba. Las redirecciones van en `Location` relativo
  (`App\Support\Rutas`) y los formularios se mandan a sí mismos. `getPathInfo()`
  es igual dentro y fuera del proxy, y por eso la cuenta sale sin saber nada de
  él.
- **`/setup` se reabre solo si la conexión guardada dejó de servir.** Reconfigurar
  es cosa de la app, salvo cuando la app no puede: para entrar hace falta el
...
Desde la 0.47.0 un servidor se actualiza solo, desde **GitHub Releases**, sin
que nadie clone ni copie nada. Lo que hay que saber antes de tocar nada:

- **Un repositorio y N instalaciones, también al actualizar.** Todas las
  empresas se bajan lo mismo del mismo sitio. La instalación se distingue por
  lo que vive fuera del código, y por eso actualizar no lo toca: el paquete
  trae exactamente las carpetas de `bin/deploy.sh` y ni `.env` ni `storage/`
  están entre ellas.
- **Se publican tres piezas y sólo se bajan las que hacen falta.** El nombre
  del paquete de dependencias lleva dentro el sha256 del `composer.lock`
  (`vendor-9ea5350edab5.tgz`), así que el servidor sabe **sin preguntar nada**
  si el `vendor` que tiene ya es el que le toca. Se publica en todas las
  versiones aunque no haya cambiado, para que la regla del cliente no tenga
  excepciones. La actualización corriente pesa 6,4 MB en vez de 24.
- **El `vendor` viaja empaquetado, no se instala en casa del cliente.** Lo que
  se probó aquí es lo que corre allá —un `composer install` en otra fecha puede
  resolver otra cosa— y el servidor de un cliente puede tener packagist cortado
  por el cortafuegos, que es quedarse a medias en el peor sitio posible.
- **Nada se escribe hasta que está todo bajado y comprobado**, con el sha256
  que calcula GitHub al subir cada archivo. Sin eso, un proxy que contesta su
  propia página de error con código 200 acabaría descomprimido encima del
  servidor.
- **Volver atrás es parte de actualizar.** `ventas:actualizar --a=0.46.0`
  instala una versión anterior, que sigue publicada. Sin eso, una versión mala
  en casa de un cliente no se arregla hasta la siguiente. (El argumento es
  `--a` y no `--version`: ése lo tiene cogido artisan.)
- **Los paquetes se construyen sin la entrada «.».** `PharData` —con lo que
  descomprime el cliente, para no exigirle `exec` a nadie— se niega a extraerla
  («Cannot extract "."»). De ahí que `bin/publicar-version.sh` nombre las
  carpetas una a una en vez de empaquetar el directorio.
- **El remate va en un PHP recién arrancado.** `migrate`, `config:clear` y
  `view:cache` se lanzan con `exec` porque el proceso que descomprimió lleva en
  memoria el cargador de clases de la versión **vieja**, y no sabría encontrar
  un paquete que acaba de llegar. Si el servidor tiene `exec` apagado se hace
  ahí mismo, que funciona salvo en ese caso.
- **No hay tercera página HTML.** Actualizar se hace con la app ya instalada,
  así que va donde va todo lo administrativo: en la app, con rol admin. El
  comando `ventas:actualizar` es el camino para el día en que la app no abra,
  que es justo cuando más falta hace.
- **Nunca se actualiza solo.** Siempre lo manda una persona.
- **Lo que tarda se sigue preguntando, no esperando.** El POST puede pasarse
  del plazo del teléfono; el estado se anota en `storage/app/private/actualizacion.json`
  y la pantalla vuelve a preguntarlo hasta que diga en qué quedó. Si la
  actualización deja el servidor a medias, ese archivo sigue ahí y dice por
  dónde.

```bash
ssh srv "cd C:\xampp\htdocs\venta-softland && C:\xampp\php\php.exe artisan ventas:actualizar --comprobar"
```
